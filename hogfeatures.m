...
close all;
H3=zeros(322,81);
srcFile=dir('C:\Users\HP\Desktop\Final_Year_Project\breast_images_final\*.pgm');
for i=1:length(srcFile)
    fn=strcat('C:\Users\HP\Desktop\Final_Year_Project\breast_images_final\',srcFile(i).name);
    Im=imread(fn);
    Im=ProcessImage(Im);
    %figure,imshow(I);
    if i==2
    ...
    hx = [-1,0,1];
    hy = -hx';
    grad_xr = imfiltrcx(double(Im),hx);
    grad_yu = imfiltrcy(double(Im),hy);
